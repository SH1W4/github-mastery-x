...
export class GitHubMasteryPaymentGateway {
    ...
    async processPayPerUse(userAddress, action, quantity = 1) {
        ...
        console.log(
            chalk.yellow(`💳 Processando pagamento: ${cost} GHMAS para ${action}`)
        );
    ...
    async stakeTokens(userAddress, amount) {
        ...
        if (!hasBalance) {
            throw new Error(
                `Saldo insuficiente para stake. Necessário: ${amount} GHMAS`
            );
        }
...
export async function demonstratePaymentFlow() {
    ...
    try {
        ...
        const purchase = await gateway.purchaseSubscription(
            userAddress,
            'developer',
            1
        );
        console.log(
            `Assinatura válida até: ${purchase.expiryDate.toLocaleDateString()}\n`
        );
