...
class LeadScoringSystem {
        ...
        emails: [
          {
            ...
            subject: 'Welcome to GitHub Agent v2.0',
          },
          {
            delay: 3,
            template: 'technical_deep_dive',
            subject: 'Under the Hood: How AI Powers 10x Speed',
          },
          {
            delay: 7,
            template: 'performance_comparison',
            subject: 'See Why Developers Choose Agent v2.0',
          },
          {
            delay: 14,
            template: 'early_access_offer',
        ...
        emails: [
          {
            ...
            subject: 'Calculate Your ROI with GitHub Agent',
          },
          {
            delay: 2,
            template: 'case_study',
            subject: 'How TechCorp Reduced Deploy Time by 85%',
          },
          {
            delay: 5,
            template: 'schedule_call',
            subject: 'Ready for a Technical Discussion?',
          },
          {
            delay: 10,
            template: 'pilot_program',
            subject: 'Exclusive 30-Day Pilot Program',
          },
        ],
      },
      manager: {
        ...
        emails: [
          ...
          {
            delay: 3,
            template: 'team_case_study',
            subject: 'Real Results from Development Teams',
          },
  ...
  trackEvent(eventType, data = {}) {
    ...
    window.dispatchEvent(
      new CustomEvent('leadScoreUpdate', {
        detail: {
          score: this.leadData.score,
  ...
  trackScrollDepth(percent) {
    ...
    const threshold = thresholds.find(
      t => percent >= t && !this.leadData.events.some(e => e.type === `scroll_${t}`)
    );
  ...
  checkTimeBasedScoring() {
    ...
    timeThresholds.forEach(threshold => {
      if (
        ...
        !this.leadData.events.some(e => e.type === `time_${threshold}s`)
      ) {
        const scoreKey =
          threshold < 60 ? `${threshold}s` : `${Math.floor(threshold / 60)}m`;
  ...
  triggerEmailSequence(sequenceType, data) {
    ...
    const alreadyTriggered = this.leadData.events.some(
      e => e.type === 'email_sequence_triggered' && e.data.sequenceType === sequenceType
    );
