...
async function checkMCP() {
  try {
    ...
    if (fs.existsSync('mcp-config.json')) {
      ...
      logger.info(
        `✅ Configuração MCP - OK (${Object.keys(mcpConfig.mcpServers || {}).length} servidores definidos)`
      );
    ...
    try {
      ...
      if (lines.length > 1) {
        logger.info(
          `✅ Processos Node.js - OK (${lines.length - 1} processos em execução)`
        );
      } else {
    ...
    if (fs.existsSync('mcp-config.json')) {
      ...
      if (mcpConfig.vireonIntegration && mcpConfig.vireonIntegration.enabled) {
        ...
        logger.info(
          `📊 Nível de awareness: ${mcpConfig.vireonIntegration.consciousnessLevel || 'basic'}`
        );
      } else {
