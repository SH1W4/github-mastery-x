...
export class GitHubMCPEcosystemAdapter extends EventEmitter {
  ...
  async connectToEcosystem() {
    try {
      ...
      this.logger.info(`Conectando ao ecossistema MCP em ${this.config.ecosystemEndpoint}...`);
      ...
      const githubRules = [
        {
          ...
          action: async (context, result) => {
            if (result?.rate_limit) {
              this.recordMetric('github_rate_limit_remaining', result.rate_limit.remaining);
  ...
  async executeTool(name, params) {
    try {
      ...
      this.emit('tool_executed', { name, params, result: processedResult, duration });
