...
export async function manageSubscription(options) {
  ...
  if (!userAddress) {
    console.log(
      chalk.yellow(
        '⚠️  Wallet address não configurado. Configure USER_WALLET_ADDRESS no .env'
      )
    );
    return;
  }
  ...
  try {
    if (options.check) {
      ...
      if (subscription.isActive) {
        ...
        console.log(
          chalk.cyan(`📅 Válida por mais ${subscription.remainingDays} dias`)
        );
      } else {
    ...
    if (options.buy) {
      ...
      console.log(
        chalk.blue(`🛒 Comprando assinatura ${tier} por ${months} mês(es)...`)
      );

      const result = await paymentGateway.purchaseSubscription(
        userAddress,
        tier,
        months
      );
      ...
      if (result.success) {
        ...
        console.log(
          chalk.cyan(`📅 Válida até: ${result.expiryDate.toLocaleDateString()}`)
        );
...
export async function manageStaking(options) {
  ...
  try {
    ...
    if (options.rewards) {
      ...
      console.log(
        chalk.cyan(`🥩 Tokens em stake: ${rewards.stakedAmount.toFixed(2)} GHMAS`)
      );
      console.log(chalk.cyan(`⏰ Duração: ${rewards.stakingDuration} dias`));
      console.log(
        chalk.cyan(
          `💰 Recompensas pendentes: ${rewards.pendingRewards.toFixed(4)} GHMAS`
        )
      );
